...
setup_host_flavor() {
  ...
  case "$flavor" in
    agave)
      ...
      ansible_in_control "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i '$CONTAINER_INVENTORY' '$playbook' --limit '$host' $base_extra -e agave_version=$AGAVE_VERSION"
      ;;
    jito-shared)
      playbook="$CONTAINER_REPO_ROOT/ansible/playbooks/pb_setup_validator_jito_v2.yml"
      ansible_in_control "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i '$CONTAINER_INVENTORY' '$playbook' --limit '$host' $base_extra -e jito_version=$JITO_VERSION"
      ;;
    jito-cohosted)
      playbook="$CONTAINER_REPO_ROOT/ansible/playbooks/pb_setup_validator_jito_v2.yml"
      ansible_in_control "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i '$CONTAINER_INVENTORY' '$playbook' --limit '$host' $base_extra -e jito_version=$JITO_VERSION"
    ...
    jito-bam)
      ...
      if [[ -n "$BAM_JITO_VERSION_PATCH" ]]; then
        ansible_in_control "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i '$CONTAINER_INVENTORY' '$playbook' --limit '$host' $base_extra -e jito_version=$BAM_JITO_VERSION -e jito_version_patch=$BAM_JITO_VERSION_PATCH"
      else
        ansible_in_control "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i '$CONTAINER_INVENTORY' '$playbook' --limit '$host' $base_extra -e jito_version=$BAM_JITO_VERSION"
      fi
      ;;
    *)
      echo "Unsupported flavor: $flavor" >&2
...
assert_host_client() {
  ...
  output="$(
    ansible_in_control "ANSIBLE_HOST_KEY_CHECKING=False ansible '$host' -i '$CONTAINER_INVENTORY' -u '$OPERATOR_USER' -b -m shell -a \"$version_cmd\" -o"
  )"
...
assert_host_validator_runtime() {
  ...
  ansible_in_control "ANSIBLE_HOST_KEY_CHECKING=False ansible '$host' -i '$CONTAINER_INVENTORY' -u '$OPERATOR_USER' -b -m shell -a \"$service_cmd\" -o" >/dev/null
  ansible_in_control "ANSIBLE_HOST_KEY_CHECKING=False ansible '$host' -i '$CONTAINER_INVENTORY' -u '$OPERATOR_USER' -b -m wait_for -a 'host=127.0.0.1 port=8899 timeout=30 state=started' -o" >/dev/null
}
...
assert_swap_identity_state() {
  ...
  ansible_in_control "ANSIBLE_HOST_KEY_CHECKING=False ansible '$SOURCE_HOST' -i '$CONTAINER_INVENTORY' -u '$OPERATOR_USER' -b -m shell -a \"$source_cmd\" -o"
  ansible_in_control "ANSIBLE_HOST_KEY_CHECKING=False ansible '$DESTINATION_HOST' -i '$CONTAINER_INVENTORY' -u '$OPERATOR_USER' -b -m shell -a \"$destination_cmd\" -o"
}

echo "[hot-swap] Preparing host prerequisites..." >&2
ansible_in_control "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i '$CONTAINER_INVENTORY' '$CONTAINER_REPO_ROOT/test-harness/ansible/pb_prepare_hot_swap_test_hosts.yml' --limit '$SOURCE_HOST,$DESTINATION_HOST' -e target_hosts='$SOURCE_HOST,$DESTINATION_HOST' -e operator_user=$OPERATOR_USER"
