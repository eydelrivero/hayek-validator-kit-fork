...
load_manual_state_if_present() {
  if [[ ! -r "$STATE_FILE" ]]; then
    ...
    echo "[manual-teardown] manual state file not found, continuing with work-root scan: $STATE_FILE" >&2
    return 0
  fi
...
cleanup_pid_file() {
  ...
  if [[ -n "${STOPPED_PIDS["$pid"]+x}" ]]; then
    echo "[manual-teardown] ${label}: pid ${pid} already handled via ${STOPPED_PIDS["$pid"]}" >&2
    return 0
  fi
...
if (( ${#PID_FILE_TARGETS[@]} == 0 )); then
  echo "[manual-teardown] no harness-owned VM pid files found" >&2
else
...
echo "[manual-teardown] stopped ${cleanup_count} harness process(es)" >&2
